...
function initCardCarousel() {
  document
    .querySelectorAll(".section-cards .carousel-wrapper")
    .forEach((wrapper) => {
      ...
      // Cria os dots conforme o total de slides visíveis
      function createDots() {
        dotsContainer.innerHTML = "";
        const totalSlides = Math.ceil(items.length / visibleItems);

        if (visibleItems > 1) {
          dotsContainer.style.display = "none"; // No desktop, sem dots
          return;
        } else {
          dotsContainer.style.display = "flex"; // Mobile mostra dots
        }
      ...
      function updateDots() {
        if (visibleItems > 1) return; // desktop sem dots
      ...
      // Limpa e ajusta o index para o limite correto
      function clampIndex() {
      ...
      function setPositionByIndex() {
        currentTranslate =
          -currentIndex * (carousel.offsetWidth / visibleItems);
      ...
      function init() {
        ...
        currentIndex = 0;
        if (visibleItems > 1) {
          // Desktop: usa translateX em porcentagem
          carousel.style.transition = "transform 0.4s ease-in-out";
          carousel.style.transform = `translateX(-${
            currentIndex * slideWidthPercent
          }%)`;
        } else {
          // Mobile: usa pixels para drag
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  initScrollColumns();
  initHamburgerMenu();
  initCardCarousel(".section-cards .carousel-wrapper");
});
